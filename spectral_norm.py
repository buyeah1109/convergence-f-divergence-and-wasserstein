...
class SpectralNorm:
    ...
    def __init__(self, name: str = 'weight', n_power_iterations: int = 1, dim: int = 0, eps: float = 1e-12, lip: float = 0.9) -> None:
        self.name = name
        self.dim = dim
    ...
    @staticmethod
    def apply(module: Module, name: str, n_power_iterations: int, dim: int, eps: float, lip: float) -> 'SpectralNorm':
        for k, hook in module._forward_pre_hooks.items():
            if isinstance(hook, SpectralNorm) and hook.name == name:
                raise RuntimeError("Cannot register two spectral_norm hooks on "
                                   "the same parameter {}".format(name))

        fn = SpectralNorm(name, n_power_iterations, dim, eps, lip)
...
def spectral_norm_flex(module: T_module,
                  name: str = 'weight',
                  n_power_iterations: int = 1,
                  eps: float = 1e-12,
                  dim: Optional[int] = None,
                  lip: float = 0.9) -> T_module:
    ...
    if dim is None:
        if isinstance(module, (torch.nn.ConvTranspose1d,
            ...
            dim = 0
    SpectralNorm.apply(module, name, n_power_iterations, dim, eps, lip)
    return module
